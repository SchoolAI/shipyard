...
export function ChangesView({
	...
	const selected =
		linkedPRs.find((pr) => pr.prNumber === selectedPR) ?? linkedPRs[0] ?? null;
	...
	const isViewingRemote =
		selectedMachine !== null && selectedMachine !== localMachineId;
	const selectedSnapshot = selectedMachine
		? changeSnapshots.get(selectedMachine)
		: undefined;
	const hasNoLocalRepo =
		localChanges?.available === false && localChanges.reason === "no_cwd";
	...
	const workingDirectory =
		source === "local"
			? isViewingRemote && selectedSnapshot
				...
				: metadata.origin?.platform === "claude-code" ||
						metadata.origin?.platform === "unknown"
					? metadata.origin.cwd
					: undefined
			: undefined;
					...
					{isViewingRemote && selectedSnapshot ? (
						...
						<Alert status="default">
							<Alert.Content>
								...
								<Alert.Description>
									Select a machine from the dropdown above to view their
									changes.
								</Alert.Description>
							</Alert.Content>
						</Alert>

...
const REGISTRY_PORT =
	import.meta.env.VITE_REGISTRY_PORT || DEFAULT_REGISTRY_PORTS[0];
		...
		<div className="flex flex-col items-center gap-4 py-6">
			...
			<div className="text-center">
				...
				<p className="text-xs text-muted-foreground mt-1">
					Share the plan URL to connect with collaborators, or download the
					handoff file to share manually.
				</p>
			</div>
			<Button
				variant="secondary"
...
function FilePickerState({
	...
	return (
		<div className="flex flex-col items-center gap-4 py-6">
			...
			<div className="text-center">
				<p className="text-sm font-medium text-foreground">
					Select your conversation file
				</p>
				<p className="text-xs text-muted-foreground mt-1">
					Choose your Claude Code session file (.jsonl) or a previously
					downloaded handoff file (.a2a.json)
				</p>
			</div>
			<input
				ref={fileInputRef}
...
export function HandoffConversationDialog({
	...
	const { exportToFile, sendToPeer, progress, isProcessing } =
		useConversationTransfer(planId, ydoc, rtcProvider);
	...
	async function handleDownloadHandoff() {
		...
		if (result.success) {
			...
			toast.success(
				`Handed off ${result.messageCount} messages to ${result.filename}`,
			);
			onClose();
		} else {
		...
		try {
			...
			const success = await sendToPeer(peer.webrtcPeerId, a2aMessages, {
				onComplete: () => {
					...
					toast.success(
						`Handed off ${a2aMessages.length} messages to ${peer.name}`,
					);
					onClose();
				},
				onError: (error) => {

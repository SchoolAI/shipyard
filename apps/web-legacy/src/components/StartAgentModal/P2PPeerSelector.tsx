		...
		<div className="space-y-3">
			<Alert status="default">
				<Alert.Content>
					...
					<Alert.Description>
						You can launch the agent via a connected peer that has daemon
						access.
					</Alert.Description>
				</Alert.Content>
			</Alert>
			...
			<Card className="p-3">
				<RadioGroup
					...
					onChange={(value) =>
						onPeerSelect(typeof value === "string" ? value : null)
					}
					isDisabled={isDisabled}
				>
					<Label className="text-sm font-medium mb-2">
						Select a peer to launch via:
					</Label>
			...
			{!selectedPeerId && (
				<p className="text-xs text-muted-foreground">
					Select a peer above, or the task will be created without launching an
					agent.
				</p>
			)}
		</div>

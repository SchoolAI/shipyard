 ...
 */
function useDeliverablesViewState(
	...
	const enrichedDeliverables = useMemo<EnrichedDeliverable[]>(
		() =>
			deliverables.map((d) => ({
				deliverable: d,
				artifact: d.linkedArtifactId
					? artifactMap.get(d.linkedArtifactId)
					: undefined,
			})),
	...
	const { useDeliverablesView, completedCount, totalCount } = useMemo(() => {
		...
		const itemsToShow = shouldUseDeliverablesView
			? enrichedDeliverables
			: artifacts;
		const completed = shouldUseDeliverablesView
			? enrichedDeliverables.filter((d) => d.deliverable.linkedArtifactId)
					.length
			: artifacts.filter((a) =>
					a.storage === "github" ? a.url : a.localArtifactId,
				).length;
...
function CompletionBanner({ metadata }: { metadata: PlanMetadata }) {
	...
	const completedDate = new Date(metadata.completedAt).toLocaleDateString(
		undefined,
		{
			month: "short",
			day: "numeric",
		...
		<div className="h-full overflow-y-auto">
			<div className="max-w-4xl mx-auto p-4 md:p-6">
				...
				<div className="flex items-center justify-between mb-6">
					...
					<Chip
						color={getCompletionChipColor(totalCount, completedCount)}
						variant="soft"
						size="sm"
					>
						{completedCount} / {totalCount}{" "}
						{useDeliverablesView ? "completed" : "attached"}
					</Chip>
				</div>
				...
				{canComplete && (
					<div className="mt-6 flex justify-end">
						<Button
							...
							onPress={() =>
								handleTaskCompletion(ydoc, identity, onRequestIdentity)
							}
						>
							Mark Complete
						</Button>
		...
		return (
			<Alert status="success" className="mt-4">
				...
				<Alert.Content>
					...
					<Alert.Description>
						All deliverables {useDeliverablesView ? "completed" : "attached"}.
						You can mark this task as complete.
					</Alert.Description>
				</Alert.Content>
			</Alert>
		);
			...
			<Alert status="accent" className="mt-4">
				...
				<Alert.Content>
					...
					<Alert.Description>
						...
						{useDeliverablesView ? "completed" : "attached"}. Waiting for
						remaining uploads.
					</Alert.Description>
				</Alert.Content>
			</Alert>

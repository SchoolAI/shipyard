...
export function CommentGutter({
	...
	const { threads, addReply, createThread, toggleResolved, deleteThread } =
		useThreads(ydoc, {
	...
	const [threadHeights, setThreadHeights] = useState<ThreadHeightMap>(
		new Map(),
	);
	...
	useEffect(() => {
		const handleClickOutside = (e: MouseEvent) => {
			if (
				...
				!gutterRef.current.contains(e.target)
			) {
				setActiveThreadId(null);
			}
		};
					...
					{positionedThreads.map(({ thread, blockId, adjustedY }) => (
						...
						>
							<ThreadCard
								...
								onScrollToBlock={
									blockId ? () => handleScrollToBlock(blockId) : undefined
								}
								onReply={
									userId ? (body) => handleReply(thread.id, body) : undefined
								}

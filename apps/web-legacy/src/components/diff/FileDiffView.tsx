...
function arePropsEqual(
	prev: FileDiffViewProps,
	next: FileDiffViewProps,
): boolean {
		...
		return (
			<Alert status="warning">
				<Alert.Content>
					...
					<Alert.Description>
						The diff for <code>{filename}</code> is not available (binary file,
						too large, or no content changes).
					</Alert.Description>
				</Alert.Content>
			</Alert>
		);
			...
			<Card.Header className="flex flex-row items-center justify-between">
				...
				<ButtonGroup size="sm" variant="tertiary">
					<Button
						...
						className={
							viewMode === "unified" ? "bg-primary/10 text-primary" : ""
						}
					>

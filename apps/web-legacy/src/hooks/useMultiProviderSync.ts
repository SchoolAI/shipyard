...
export function useMultiProviderSync(
	...
	const handleConnectionTimeout = useCallback(
		(mounted: { current: boolean }, updateSyncState: () => void) => {
			...
			if (!wsConnected && !hasP2PPeers) {
				...
				errorRef.current =
					"Connection timeout - check network or start MCP server";
	...
	useEffect(() => {
		...
		const visibilityGracePeriodTimeoutRef: {
		...
		idbProvider.whenSynced.then(async () => {
			...
			if (docName !== "plan-index") {
				window.dispatchEvent(
					new CustomEvent("indexeddb-plan-synced", {
						detail: { planId: docName },
					}),
				);
			}

 ...
 */
function hasAnchorBlockId(thread: Thread): thread is Thread & { anchorBlockId: string } {
 ...
 */
export function useThreads(ydoc: Y.Doc, options: UseThreadsOptions = {}): UseThreadsResult {
  const { includeResolved = false } = options;

  const [threads, setThreads] = useState<Thread[]>([]);
  const [threadsByBlock, setThreadsByBlock] = useState<Map<string, Thread[]>>(new Map());
  ...
  const updateThreads = useCallback(() => {
    ...
    const filteredThreads = includeResolved ? allThreads : allThreads.filter((t) => !t.resolved);
  ...
  const createThread = useCallback(
    (blockId: string, body: string, userId: string, selectedText?: string): string | null => {
      const threadsMap = ydoc.getMap(YDOC_KEYS.THREADS);

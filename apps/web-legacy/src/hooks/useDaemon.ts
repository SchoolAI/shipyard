...
export type AgentStartCallback = (
	taskId: string,
	result:
		| { success: true; pid: number; sessionId?: string }
...
export function useDaemon(): UseDaemonReturn {
		...
		switch (message.type) {
			case "started": {
				setState((prev) => ({
					...
					agents: [
						...prev.agents,
						{ taskId: message.taskId, pid: message.pid },
					],
					lastStarted: {
			...
			default: {
				...
				throw new Error(
					`Unhandled message type: ${JSON.stringify(_exhaustive)}`,
				);
			}
		}
	}, []);
		...
		const connect = () => {
			...
			try {
				...
				ws.onopen = () => {
					...
					ws.send(
						JSON.stringify({ type: "list-agents" } satisfies ClientMessage),
					);
				};
	...
	const onAgentStart = useCallback(
		(taskId: string, callback: AgentStartCallback): (() => void) => {

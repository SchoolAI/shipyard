...
const OG_PROXY_WORKER_URL_PROD =
...
function OGPreviewCard({ plan, workerUrl }: OGPreviewCardProps) {
	...
	const completedCount =
		plan.deliverables?.filter((d) => d.linkedArtifactId).length ?? 0;
			...
			<div className="max-w-6xl mx-auto">
				...
				<div className="mb-6">
					...
					<p className="text-muted-foreground">
						Test how your snapshot URLs will appear when shared on different
						platforms.
					</p>
				</div>
								...
								<Tooltip>
									<Tooltip.Trigger>
										<Button
											type="button"
											variant="secondary"
											onPress={useCurrentPlan}
										>
											Use Current URL
										</Button>
									</Tooltip.Trigger>
									<Tooltip.Content>
										Use the ?d= parameter from the current page URL
									</Tooltip.Content>
								</Tooltip>
							</div>
						</div>
				...
				{decodedPlan && (
					<details className="mt-8">
						...
						<Surface
							variant="secondary"
							className="rounded-lg p-4 mt-2 overflow-auto"
						>
				...
				{!decodedPlan && !error && (
					<div className="text-center py-12">
						...
						<p className="text-sm text-muted-foreground max-w-md mx-auto">
							Paste a snapshot URL above to see how it will appear when shared
							on GitHub, Discord, Slack, and Teams.
						</p>
					</div>
				)}
			</div>

...
const ActivePlanSyncContext = createContext<ActivePlanSyncContextValue | undefined>(undefined);
...
export function ActivePlanSyncProvider({ children }: { children: ReactNode }) {
  ...
  return (
    <ActivePlanSyncContext.Provider
      value={{ activePlanId, syncState, setActivePlanSync, clearActivePlanSync }}
    >
      {children}
    </ActivePlanSyncContext.Provider>
  );
...
export function useActivePlanSync() {
  ...
  if (!context) {
    throw new Error('useActivePlanSync must be used within ActivePlanSyncProvider');
  }
  return context;
}

...
export function KanbanPage() {
	...
	const peekPlan = peekPlanId
		? allPlans.find((p) => p.id === peekPlanId)
		: null;
	...
	const handleApprove = useCallback(async () => {
		...
		const now = Date.now();
		const reviewedBy =
			githubIdentity?.displayName || githubIdentity?.username || "Unknown";
	...
	const announcements: Announcements = {
		...
		onDragOver({ active, over }) {
			...
			if (over) {
				...
				const status = String(
					columnData?.status || columnData?.column?.id || "unknown",
				);
			...
			<header className="flex items-center justify-between px-4 py-3 border-b border-separator shrink-0">
				...
				<div className="flex items-center gap-2">
					<Tooltip>
						...
						<Tooltip.Content>
							{hideEmptyColumns ? "Show" : "Hide"} empty columns
						</Tooltip.Content>
					</Tooltip>
				</div>
			</header>
				...
				{selectedPlanId && panelMetadata ? (
					<>
						<PlanPanelHeader
							...
							onClose={handleClosePanel}
							onExpand={() =>
								cycleWidth(panelWidth === "peek" ? "expand" : "collapse")
							}
							onFullScreen={() => {
								if (selectedPlanId) {
									setSidebarCollapsed(true);

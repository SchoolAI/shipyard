...
export function SearchPage() {
	...
	const { statusFilters, toggleStatusFilter, tagFilters, toggleTagFilter } =
		useViewFilters();
	...
	const filteredPlans = useMemo(() => {
		...
		if (tagFilters.length > 0) {
			plans = plans.filter((plan) =>
				plan.tags?.some((tag) => tagFilters.includes(tag)),
			);
		}
	...
	return (
		...
		>
			...
			>
				...
				{/* Header with search */}
				<div
					className={`border-b border-separator shrink-0 ${selectedPlanId ? "p-4" : "mb-4"}`}
				>
				...
				{/* Results */}
				<div
					className={`flex-1 overflow-y-auto ${selectedPlanId ? "p-2" : ""}`}
				>
							...
							<div className="flex items-center justify-center py-12">
								<div className="text-center">
									...
									<p className="text-muted-foreground">
										Type to search tasks or use filters
									</p>
								</div>
							</div>
						)}
						...
						sortedPlans.length > 0 && (
							<>
								<p className="text-sm text-muted-foreground px-2 mb-2">
									{sortedPlans.length}{" "}
									{sortedPlans.length === 1 ? "result" : "results"}
								</p>
								<ListBox

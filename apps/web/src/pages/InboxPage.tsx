...
} from "@/components/InlinePlanDetail";
				...
				<Tooltip>
					<Tooltip.Trigger>
						<Button
							...
							onPress={() =>
								isRead ? onMarkUnread(plan.id) : onDismiss(plan.id)
							}
							className="w-8 h-8"
						>
							{isRead ? (
...
function groupInboxEvents(
	...
	const filterByReadState = (items: InboxEventItem[]) => {
		...
		return items.filter(
			(item) => item.isUnread || item.plan.id === selectedPlanId,
		);
	};
	...
	return {
		...
		mentions: filterByReadState(
			eventBasedInbox.filter(
				(e: InboxEventItem) =>
					e.event.type === "comment_added" && e.event.data?.mentions,
			),
		),
		readyToComplete: filterByReadState(
			eventBasedInbox.filter(
				(e: InboxEventItem) =>
					e.event.type === "deliverable_linked" && e.event.data?.allFulfilled,
			),
		),
	};
}

function usePanelUrlSync(
	selectedPlanId: string | null,
...
function useInputRequestEventListener(
	setCurrentInputRequest: React.Dispatch<
		React.SetStateAction<AnyInputRequest | null>
	>,
...
function navigateToAdjacentItem(
	...
	const isValidIndex =
		direction === "next" ? currentIndex < plans.length - 1 : currentIndex > 0;
...
function updatePlanToInProgress(
	ydoc: Y.Doc,
	now: number,
	actor: string,
	reviewedBy: string,
...
function updateIndexToInProgress(
	indexDoc: Y.Doc,
	planId: string,
	now: number,
): void {
...
function updatePlanToChangesRequested(
	...
	transitionPlanStatus(
		ydoc,
		{ status: "changes_requested", reviewedAt: now, reviewedBy },
		actor,
	);
}

function updateIndexToChangesRequested(
	indexDoc: Y.Doc,
	planId: string,
	now: number,
): void {
...
function getInboxStatusMessage(
	totalItems: number,
					...
					<div className="space-y-2 px-3">
						{pendingRequests.map((request) => (
							<InputRequestInboxItem
								...
								planTitle={
									request.planId ? getPlanTitle(request.planId) : undefined
								}
...
function InboxDetailPanel({
	...
	return (
		<div className="flex flex-col h-full overflow-hidden">{detailContent}</div>
	);
}
...
export function InboxPage() {
	...
	const eventBasedInbox = useInboxEvents(
		allOwnedPlans,
	...
	const [currentInputRequest, setCurrentInputRequest] =
		useState<AnyInputRequest | null>(null);
	...
	useInputRequestEventListener(
		setCurrentInputRequest,
		setInputRequestModalOpen,
	);
	...
	const getNextSelectedId = useCallback(
		(currentIndex: number): string | null =>
			getNextOrPrevId(sortedInboxPlans, currentIndex),
		[sortedInboxPlans],
	);
	...
	if (isLoading) {
		return (
			<TwoColumnSkeleton itemCount={3} showActions={true} titleWidth="w-20" />
		);
	}
	...
	const statusMessage = getInboxStatusMessage(
		totalInboxItems,
		allInboxPlans.length,
	);
	...
	return (
		...
		>
			...
			>
				{/* Offline banner */}
				{timedOut && (
					<OfflineBanner onRetry={reconnect} isReconnecting={isReconnecting} />
				)}

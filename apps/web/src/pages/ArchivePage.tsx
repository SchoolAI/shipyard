	...
	return (
		<div className="flex items-center justify-between gap-3 w-full py-2">
			<div className="flex flex-col gap-1 flex-1 min-w-0">
				<span className="font-medium text-foreground truncate opacity-70">
					{plan.title}
				</span>
				<div className="flex items-center gap-2 flex-wrap">
					<span className="text-xs text-muted-foreground">
						Archived{" "}
						{formatRelativeTime(plan.deleted ? plan.deletedAt : plan.updatedAt)}
					</span>
					{/* Show first 3 tags */}
...
export function ArchivePage() {
	...
	const sortedArchivedPlans = useMemo(() => {
		return [...archivedPlans].sort(
			(a, b) =>
				(b.deleted ? b.deletedAt : b.updatedAt) -
				(a.deleted ? a.deletedAt : a.updatedAt),
		);
	}, [archivedPlans]);
	...
	useKeyboardShortcuts({
		...
		onNextItem: useCallback(() => {
			...
			const currentIndex = sortedArchivedPlans.findIndex(
	...
	if (isLoading) {
		return (
			<TwoColumnSkeleton itemCount={3} showActions={false} titleWidth="w-20" />
		);
	}

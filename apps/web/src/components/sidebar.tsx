...
import { getSidebarCollapsed, setSidebarCollapsed } from '@/utils/ui-preferences';
...
function CollapsedSidebar({ inboxCount, onToggle, onNavigate }: CollapsedSidebarProps) {
  const navigate = useNavigate();
...
export function Sidebar({ onNavigate, inDrawer = false }: SidebarProps) {
  ...
  const collapsedContent = (
    <CollapsedSidebar inboxCount={inboxCount} onToggle={handleToggle} onNavigate={onNavigate} />
  );
  ...
  const content = (
    <>
      ...
      <div className="px-3 py-2 border-b border-separator space-y-1">
        <NavItem
          icon={<Inbox className={`w-4 h-4 ${inboxCount > 0 ? 'text-warning' : ''}`} />}
          label="Inbox"
          href="/inbox"

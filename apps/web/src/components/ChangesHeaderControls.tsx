...
function getEffectiveLocalData(state: ChangesViewState): {
	...
	const isViewingRemote =
		selectedMachineId !== null && selectedMachineId !== localMachineId;
	if (isViewingRemote) {
...
export function ChangesHeaderControls({
	...
	const { identity, hasRepoScope, startAuth, requestRepoAccess } =
		useGitHubAuth();
	...
	const classifyPublishError = useCallback(
		(
			...
			if (
				status === 403 &&
				responseBody.message?.toLowerCase().includes("rate limit")
			) {
				return {
					type: "rate_limited",
					message:
	...
	const handlePublish = useCallback(async () => {
		...
		try {
			...
			if (!response.ok) {
				...
				const classifiedError = classifyPublishError(
					response.status,
					errorData,
				);
				...
				if (
					classifiedError.type === "rate_limited" ||
		...
		return (
			<div className="flex flex-wrap items-center gap-2">
				...
				{hasPRs && selectedPR && (
					<ButtonGroup size="sm" variant="ghost" hideSeparator>
						<Button
							...
							className={
								source === "local" ? "font-medium" : "text-muted opacity-70"
							}
						>
						...
						<Button
							...
							className={
								source === "pr" ? "font-medium" : "text-muted opacity-70"
							}
						>
				...
				<Button
					...
					aria-label={
						source === "local" ? "Refresh local changes" : "Refresh PR files"
					}
				>
					<RefreshCw
						className={`w-3.5 h-3.5 ${
							(
								source === "local"
									? localChanges.isFetching
									: prChanges.isFetching
							)
								? "animate-spin"
								: ""
						}`}
					...
					<Dropdown.Section>
						...
						{publishError.type === "needs_repo_scope" && (
							<Dropdown.Item
								...
								textValue="Grant private repo access"
							>
						...
						{publishError.type === "needs_write_access" && (
							<Dropdown.Item
								id="github-link"
								textValue="Check access on GitHub"
							>
						...
						{(publishError.type === "network_error" ||
							publishError.type === "rate_limited") && (
...
function MachineDropdown({
	...
	const { snapshots, localMachineId, selectedMachineId, onSelectMachine } =
		machinePicker;

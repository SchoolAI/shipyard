...
import { useFormSubmit } from '@/lib/hotkeys';
...
export function ReviewActions({
  ...
  const { hint } = useFormSubmit({
    onSubmit: () => {
      if (openPopover === 'approve') {
        handleConfirm('approve');
      } else if (openPopover === 'changes') {
        handleConfirm('request_changes');
      }
    },
    isDisabled: isSubmitting || openPopover === null,
          ...
          <Popover.Dialog>
            ...
            <div className="mt-3 space-y-3">
              ...
              <div className="relative">
                <TextArea
                  id="approve-comment"
                  placeholder={`Great work! Consider also... (${hint})`}
                  value={comment}
            ...
            <div className="mt-3 space-y-3">
              ...
              <div className="relative">
                <TextArea
                  id="changes-comment"
                  ref={textareaRef}
                  placeholder={`Please update the error handling to... (${hint})`}
                  value={comment}
                  onChange={(e) => setComment(e.target.value)}
                  rows={3}
                  className="w-full pr-12"

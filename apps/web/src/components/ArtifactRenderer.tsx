...
function BinaryArtifactViewer({
	...
	const [status, setStatus] = useState<FetchArtifactStatus | "loading">(
		"loading",
	);
	...
	if (status === "not_found") {
		return (
			<ArtifactPlaceholder filename={filename} message="Artifact not found" />
		);
	}
		...
		return (
			<Alert status="warning">
				...
				<Alert.Content>
					...
					<Alert.Description>
						This artifact is stored locally and is not accessible on this
						device. It may have been created on a different machine.
					</Alert.Description>
				</Alert.Content>
			</Alert>
		);
		...
		<Card variant="secondary" className="text-center">
			<Card.Content className="flex flex-col items-center gap-3 py-6">
				...
				<p className="text-sm text-muted-foreground">
					This artifact is from a private repository
				</p>
				{needsRepoUpgrade ? (

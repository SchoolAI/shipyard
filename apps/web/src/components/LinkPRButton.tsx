...
export function LinkPRButton({
	...
	const handleSubmit = async (e: React.FormEvent) => {
		...
		if (!targetRepo) {
			toast.error(
				"No repository specified. Set repo in Advanced or add to plan metadata.",
			);
			return;
		}
		...
		try {
			...
			const response = await fetch(
				`https://api.github.com/repos/${targetRepo}/pulls/${prNumber}`,
				{
					headers: {
			...
			const message =
				error instanceof Error ? error.message : "Failed to link PR";
			toast.error(message);
		} finally {
							...
							<Disclosure.Content>
								<Disclosure.Body>
									<TextField value={repo} onChange={setRepo} name="repo">
										...
										<Description>
											Override repository (default:{" "}
											{metadata?.repo || "none set"})
										</Description>
									</TextField>
								</Disclosure.Body>
							</Disclosure.Content>

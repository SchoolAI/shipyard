...
export function InputRequestModal({
  ...
  const isSubmitDisabledValue =
    !request ||
    request.type === 'confirm' ||
    isSubmitDisabled(isSubmitting, request, value, isOtherSelected, customInput, isNaSelected);

  useFormSubmit({
    onSubmit: () => submitButtonRef.current?.click(),
    isDisabled: isSubmitDisabledValue || !isOpen,
  });

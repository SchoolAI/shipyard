...
function convertSnapshotToLocalChanges(
...
function LocalChangesContent({
	...
	const NodeRenderer = useMemo(
		() => createFileTreeNode(setSelectedFile),
		[setSelectedFile],
	);
			...
			<Card>
				<Card.Content className="p-8 text-center">
					...
					<p className="text-muted-foreground">
						Your working directory is clean on branch{" "}
						<code className="text-xs">{data.branch}</code>
					</p>
				</Card.Content>
			</Card>
		);
		...
		<div className="flex flex-col h-full min-h-0">
			...
			>
				...
				<div className="flex flex-col h-full min-h-0 overflow-y-auto">
					{selectedFile &&
					(selectedFileData || isUntrackedFile(selectedFile)) ? (
...
function buildGroupedFileTree(
	...
	if (data.unstaged.length > 0) {
		sections.push({
			...
			isSection: true,
			category: "unstaged",
			children: buildFileTreeForStatus(
				enrichWithStats(data.unstaged),
				"unstaged",
			),
		});
	}
 ...
 */
function buildFileTreeForStatus(
	files: LocalFileChange[],
...
function createFileTreeNode(onFileClick: (path: string) => void) {
	...
	return function FileTreeNode({
		node,
		style,
	}: NodeRendererProps<FileTreeData>) {
		...
		const styles = stagingStatus
			? statusStyles[stagingStatus]
			: statusStyles.unstaged;
		...
		return (
			...
			>
				{/* Status icon */}
				<span
					className={`shrink-0 ${node.isSelected ? "text-white/80" : styles.iconClass}`}
				>
					{styles.icon}
				</span>
				...
				{file && stagingStatus !== "untracked" && (
					<div className="flex items-center gap-1.5 shrink-0">
						<span
							className={`text-xs ${node.isSelected ? "text-white/80" : "text-success"}`}
						>
							+{file.additions}
						</span>
						<span
							className={`text-xs ${node.isSelected ? "text-white/80" : "text-danger"}`}
						>
							-{file.deletions}
						</span>
					</div>

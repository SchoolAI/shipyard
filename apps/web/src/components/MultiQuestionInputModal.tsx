...
function validateEmailInput(
	...
	if (
		question.domain &&
		!email.toLowerCase().endsWith(`@${question.domain.toLowerCase()}`)
	) {
		return false;
	}
	return true;
...
function isChoiceSubmittable(
	...
	if (!otherSelected && (Array.isArray(value) ? value.length === 0 : !value))
		return false;
	return true;
}

/** Validate rating-type question submission */
function isRatingSubmittable(
	value: string | string[],
	customInput: string,
): boolean {
	const strValue = typeof value === "string" ? value : "";
 ...
 */
function isQuestionSubmittable(
	question: Question,
	state: QuestionState,
): boolean {
	const { value, customInput } = state;
		...
		>
			<Modal.Container placement="center" size="lg">
				<Modal.Dialog>
					...
					<Card
						className={
							isBlocker ? "border-2 border-danger ring-2 ring-danger/20" : ""
						}
					>
						<Card.Header>
							...
							<div className="space-y-4">
								...
								<Alert status={isBlocker ? "danger" : "warning"}>
									...
									<Alert.Content>
										...
										<Alert.Description>
											You need to sign in with GitHub to respond to this
											request. Your identity will be recorded with your
											response.
											{isBlocker &&
...
export function MultiQuestionInputModal({
	...
	const handleAnswerError = useCallback(
		(
			result: Extract<AnswerInputRequestResult, { success: false }>,
			onCloseFn: () => void,
		) => {
			const { error } = result;
			switch (error) {
				case "Request already answered": {
					const answeredBy =
						"answeredBy" in result ? result.answeredBy : undefined;
	...
	const renderConfirmQuestion = (
		...
		const showExplainInput =
			typeof value === "string" && value === "__explain__";
		return (
					...
					<div className="space-y-3">
						<TextField isDisabled={isSubmitting}>
							...
							<TextArea
								value={customInput}
								onChange={(e) =>
									updateQuestionState(index, { customInput: e.target.value })
								}
						...
						<Button
							...
							size="sm"
							onPress={() =>
								updateQuestionState(index, { value: "", customInput: "" })
							}
							isDisabled={isSubmitting}
						>
							Back
	...
	const buildBaseInputProps = (value: string | string[], index: number) => ({
		...
		setValue: (val: string | string[]) =>
			updateQuestionState(index, { value: val }),
		isSubmitting,
	});
		...
		switch (question.type) {
			case "text":
				return (
					<TextInput
						{...baseInputProps}
						request={{ ...baseRequestProps, type: "text" }}
					/>
				);
			case "multiline":
				return (
			...
			case "choice":
				return (
					<ChoiceInput
						...
						setCustomInput={(val) =>
							updateQuestionState(index, { customInput: val })
						}
					...
					<Alert status="warning">
						<Alert.Content>
							...
							<Alert.Description>
								...
								Type "{(_exhaustiveCheck as { type: string }).type}" is not
								supported.
							</Alert.Description>
						</Alert.Content>
					</Alert>
	...
	const isFormSubmittable = (): boolean => {
		if (!request || questionStates.length !== request.questions.length)
			return false;

...
class MockAwareness {
	...
	setLocalStateField(field: string, value: unknown): void {
		const current = (this.states.get(this.localClientId) || {}) as Record<
			string,
			unknown
		>;
		current[field] = value;
...
describe("useMultiProviderSync", () => {
	...
	describe("Circuit Breaker Behavior", () => {
		...
		it("should allow manual reconnection after timeout via reconnect()", async () => {
			...
			await act(async () => {
				const latestIdb =
					mockIndexeddbProviders[mockIndexeddbProviders.length - 1]!;
		...
		it("should clear timeout state when connection succeeds after previous timeout", async () => {
			...
			const newWsProvider =
				mockWebsocketProviders[mockWebsocketProviders.length - 1]!;
	...
	describe("Cleanup Behavior (Memory Leak Prevention)", () => {
		it("should remove all event listeners when unmounting", async () => {
			...
			expect(
				rtcProvider.awareness.getEventHandlerCount("change"),
			).toBeGreaterThan(0);
	...
	describe("Configuration Options", () => {
		...
		it("should use provided userName for awareness", async () => {
			renderHook(() =>
				useMultiProviderSync("test-doc", { userName: "TestUser" }),
			);
			...
			const localState = states.get(1) as {
				planStatus?: { user?: { name?: string } };
			};
	...
	describe("Document Name Changes", () => {
		it("should create new Y.Doc when docName changes", async () => {
			const { result, rerender } = renderHook(
				({ docName }) => useMultiProviderSync(docName),
				{

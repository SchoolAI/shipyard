...
export function useLocalIdentity(): UseLocalIdentityReturn {
	...
	const localIdentity = useSyncExternalStore(
		subscribeAll,
		getSnapshot,
		getServerSnapshot,
	);
	...
	const setLocalIdentity = useCallback((username: string) => {
		...
		if (username.startsWith("local:") || username.startsWith("github:")) {
			throw new Error(
				"Username cannot start with reserved prefixes (local:, github:)",
			);
		}
		...
		if (!/^[a-zA-Z0-9-]{2,39}$/.test(username)) {
			throw new Error(
				"Username must be 2-39 characters: letters, numbers, and hyphens only",
			);
		}

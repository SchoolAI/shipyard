 ...
 */
function mergeViewedByState(
	prevState: ViewedByRecord,
	newData: ViewedByRecord,
): ViewedByRecord {
	...
	for (const [planId, existingViewedBy] of Object.entries(prevState)) {
		if (!merged[planId]) {
			...
			merged[planId] = mergeViewedByTimestamps(
				existingViewedBy,
				merged[planId],
			);
		}
...
export function usePlanIndex(
	...
	const { ydoc, syncState, reconnect, isReconnecting } =
		useMultiProviderSync(PLAN_INDEX_DOC_NAME);
	...
	const [planViewedBy, setPlanViewedBy] = useState<
	 ...
	 */
	const isLoading =
		!syncState.idbSynced && !syncState.synced && !syncState.timedOut;

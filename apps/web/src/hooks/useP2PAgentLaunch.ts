 ...
 */
function extractPeersFromProvider(
...
function validateA2APayload(
	...
	const { valid: validMessages, errors: messageErrors } = validateA2AMessages(
		a2aPayload.messages,
	);
	if (messageErrors.length > 0 || validMessages.length === 0) {
		const errorMsg =
			messageErrors.map((e) => e.error).join(", ") || "no valid messages";
...
export function useP2PAgentLaunch({
	...
	const [lastResult, setLastResult] = useState<P2PAgentLaunchResult | null>(
		null,
	);
	...
	const pendingLocalLaunchesRef = useRef<
		Map<string, (response: AgentLaunchResponse) => void>
	>(new Map());

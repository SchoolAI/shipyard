...
export function createGitHubProxyRoutes(ctx: GitHubProxyContext) {
  ...
  app.get(ROUTES.PR_DIFF, async (c) => {
    ...
    if (Number.isNaN(prNum) || prNum <= 0) {
      return c.json<GitHubProxyError>({ code: 'not_found', message: 'Invalid PR number' }, 404);
    }
    ...
    try {
      ...
      return c.json<GitHubProxyError>({ code: 'github_error', message: 'GitHub API error' }, 500);
    }
  });

			...
			it("fetches URL and returns base64 content", async () => {
				...
				if (result.success) {
					expect(result.content).toBe(
						Buffer.from("url content").toString("base64"),
					);
				}
	...
	describe("validateArtifactType", () => {
		...
		describe("image type", () => {
			...
			for (const ext of validImageExtensions) {
				it(`accepts .${ext} extension`, () => {
					expect(() =>
						validateArtifactType("image", `screenshot.${ext}`),
					).not.toThrow();
				});
			}
		...
		describe("video type", () => {
			...
			for (const ext of validVideoExtensions) {
				it(`accepts .${ext} extension`, () => {
					expect(() =>
						validateArtifactType("video", `recording.${ext}`),
					).not.toThrow();
				});
			}
		...
		describe("edge cases", () => {
			it("handles case-insensitive extensions", () => {
				expect(() =>
					validateArtifactType("image", "screenshot.PNG"),
				).not.toThrow();
				expect(() =>
					validateArtifactType("image", "screenshot.Jpg"),
				).not.toThrow();
			});

			it("rejects files without extensions", () => {
				expect(() => validateArtifactType("image", "noextension")).toThrow(
					"Got: noextension",
				);
			});
			...
			it("uses last extension for multi-dot filenames", () => {
				expect(() =>
					validateArtifactType("image", "file.backup.png"),
				).not.toThrow();
			});

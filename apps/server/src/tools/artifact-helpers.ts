...
import type { Artifact, ArtifactType, GitHubArtifact, LocalArtifact } from '@shipyard/schema';
  ...
  switch (input.source) {
    ...
    case 'url': {
      ...
      try {
        ...
        return { success: true, content: Buffer.from(arrayBuffer).toString('base64') };
      } catch (error) {
 ...
 */
export async function uploadArtifactWithFallback(params: UploadParams): Promise<UploadResult> {
  const { planId, filename, content, validatedType, description, repo } = params;
 ...
 */
async function storeLocally(params: UploadParams, reason: string): Promise<UploadResult> {

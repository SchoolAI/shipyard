	...
	const daemon = spawn("node", [daemonPath], {
		...
		env: {
			...
			...(process.env.SHIPYARD_STATE_DIR && {
				SHIPYARD_STATE_DIR: process.env.SHIPYARD_STATE_DIR,
			}),
			...(process.env.SHIPYARD_WEB_URL && {
				SHIPYARD_WEB_URL: process.env.SHIPYARD_WEB_URL,
			}),
...
async function setupAutoStart(): Promise<boolean> {
	try {
		...
		logger.warn(
			{ err },
			"Failed to import auto-start module - daemon may not be built yet",
		);
		return false;
	}
}
...
export async function ensureDaemonRunning(): Promise<void> {
	...
	if (!isConfigured) {
		...
		if (success) {
			logger.info(
				"Auto-start configured successfully - daemon will survive reboots",
			);
		} else {
			logger.warn(
				"Failed to configure auto-start, falling back to manual spawn",
			);
		}
	} else {
		logger.info(
			"Auto-start already configured - daemon will auto-start on boot",
		);
	}
	...
	try {
		...
		logger.error(
			{ err },
			"Failed to spawn daemon - browser agent launching will not work",
		);
	}
}

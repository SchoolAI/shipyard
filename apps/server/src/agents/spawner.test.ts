...
import { initSpawner, spawnClaudeCode, spawnClaudeCodeWithContext, stopAgent } from './spawner.js';
...
function createMockChildProcess(pid = 12345): ChildProcess {
  ...
  // Create mock streams with EventEmitter interface
  const mockStdout = Object.assign(new EventEmitter(), {
    pipe: vi.fn(),
  });
  const mockStderr = Object.assign(new EventEmitter(), {
    pipe: vi.fn(),
  });

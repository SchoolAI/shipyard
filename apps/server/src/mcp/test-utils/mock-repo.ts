 ...
 */
export function createMockTaskMeta(overrides?: Partial<MockTaskMeta>): MockTaskMeta {
  const now = Date.now();
  return {
    id: 'test-task-123',
    title: 'Test Task',
 ...
 */
export function createMockTask(taskId: string, meta?: Partial<MockTaskMeta>): void {
  ...
  return {
    ...
    get inputRequests() {
      return {
        ...
        get: (idx: number) => {
          if (idx < 0 || idx >= task.inputRequests.length) return undefined;
          // Return a proxy that allows mutation
          const item = task.inputRequests[idx] as Record<string, unknown>;
          return {
            get status() {
            ...
            get answeredAt() {
              return item.answeredAt as number | null;
            },
            set answeredAt(v: number | null) {
              item.answeredAt = v;
            },
            get answeredBy() {
              return item.answeredBy as string | null;
            },
            set answeredBy(v: string | null) {
              item.answeredBy = v;
            },
          };
        },

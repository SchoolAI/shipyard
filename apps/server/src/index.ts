...
if (mode === 'client' || explicitHubUrl) {
  ...
  if (explicitHubUrl) {
    ...
    try {
      ...
      logger.info({ hubUrl: explicitHubUrl, hubPort }, 'Using explicit hub URL (client mode)');
    } catch (err) {
    ...
    if (!hubPort) {
      logger.error('SHIPYARD_MODE=client but no hub found on ports 32191-32199');
      process.exit(1);
    }
  }

  logger.info({ registryPort: hubPort }, 'Connecting to registry hub as client');
  await initAsClient(hubPort);
} else if (mode === 'hub') {
  /** FORCE hub mode - become hub regardless */
  const acquired = await tryAcquireHubLock();
  if (!acquired) {
    logger.error('SHIPYARD_MODE=hub but could not acquire lock (another hub running?)');
    process.exit(1);
  }

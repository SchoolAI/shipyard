...
import { createWebRtcAdapter, type WebRtcDataChannelAdapter } from './adapters/webrtc.js';
import { createWebSocketAdapter, type WsServerNetworkAdapter } from './adapters/websocket.js';
...
export function createRepo(wss: WebSocketServer): Repo {
  ...
  repo = new Repo({
    ...
    permissions: {
      visibility(_doc: DocContext, _peer: PeerContext) {
        // Allow all peers to see all docs
        // TODO: Implement fine-grained permissions based on doc type/owner
        return true;
      },
    },
  });

 ...
 */
export type RouteResult<T> = { ok: true; value: T } | { ok: false; error: Response };
 ...
 * ```
 */
export function validationErrorResponse(c: Context, zodError: ZodError): Response {
...
export async function parseJsonBody(c: Context): Promise<RouteResult<unknown>> {
  try {
    ...
    return {
      ok: false,
      error: errorResponse(c, ErrorCodes.INVALID_BODY, 'Invalid JSON body', 400),
    };
  }
}
...
export function extractBearerToken(c: Context): RouteResult<string> {
  ...
  if (!authHeader?.startsWith('Bearer ')) {
    return {
      ok: false,
      error: errorResponse(c, ErrorCodes.UNAUTHORIZED, 'Bearer token required', 401),
    };
  }
 ...
 * ```
 */
export function requireQueryParam(c: Context, param: string): RouteResult<string> {
  const value = c.req.query(param);
  if (!value) {
    return {
      ok: false,
      error: errorResponse(c, ErrorCodes.MISSING_TOKEN, `${param} query param required`, 401),
    };
  }
...
export function requireWebSocketUpgrade(c: Context): RouteResult<void> {
  ...
  if (upgradeHeader !== 'websocket') {
    return {
      ok: false,
      error: errorResponse(c, ErrorCodes.UPGRADE_REQUIRED, 'WebSocket upgrade required', 426),
    };
  }

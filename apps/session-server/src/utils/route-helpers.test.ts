...
describe('route-helpers', () => {
  describe('errorResponse', () => {
    ...
    it('creates 426 error response', async () => {
      const app = createTestApp();
      app.get('/test', (c) => {
        return errorResponse(c, ErrorCodes.UPGRADE_REQUIRED, 'Upgrade required', 426);
      });
  ...
  describe('requireQueryParam', () => {
    ...
    it('handles URL-encoded values', async () => {
      ...
      const res = await makeRequest(app, `/test?token=${encodeURIComponent('value with spaces')}`);

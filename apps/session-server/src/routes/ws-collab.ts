...
wsCollabRoute.get(ROUTES.WS_COLLAB, async (c) => {
  ...
  const payload = await validatePresignedUrlAsync(tokenResult.value, c.env.JWT_SECRET);
  if (!payload) {
  ...
  let userClaims: { sub: string; displayName: string; avatarUrl?: string | null } | undefined;
  if (userToken) {

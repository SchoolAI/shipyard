...
export class SignalingClient {
   ...
   * @throws {SignalingClientValidationError} If request/response doesn't match expected schema
   */
  async createCollab(request: CollabCreateRequest, token: string): Promise<CollabCreateResponse> {
   ...
   */
  buildCollabRoomUrl(roomId: string, presignedToken: string, userToken?: string): string {
    const wsBaseUrl = this.baseUrl.replace(/^http:/, 'ws:').replace(/^https:/, 'wss:');
...
export class AuthenticatedSignalingClient extends SignalingClient {
   ...
   * @returns Pre-signed WebSocket URL and room info
   */
  override async createCollab(request: CollabCreateRequest): Promise<CollabCreateResponse>;

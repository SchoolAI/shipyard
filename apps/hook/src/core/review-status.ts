...
function buildApprovalMessage(params: ApprovalMessageParams): string {
	const { prefix, planId, sessionToken, url, deliverables, reviewComment } =
		params;
...
async function handleApproval(
	...
	try {
		...
		logger.error(
			{ err, planId },
			"Failed to set session token, approving without it",
		);
		return {
			allow: true,
...
async function waitForReviewDecision(planId: string): Promise<ReviewDecision> {
	...
	logger.info(
		{ planId, approved: result.approved },
		"Received approval decision from server",
	);
...
async function handleUpdatedPlanReview(
	...
	logger.info(
		{ planId, approved: decision.approved },
		"Decision received via Y.Doc",
	);
	...
	logger.debug(
		{ planId },
		"Changes requested - server will manage state cleanup",
	);
...
export async function checkReviewStatus(
	...
	if (!state.found) {
		logger.info(
			{ sessionId },
			"No session state or plan content, allowing exit",
		);
		return { allow: true };
	}
	...
	logger.info(
		{ sessionId, planId, status: status.status },

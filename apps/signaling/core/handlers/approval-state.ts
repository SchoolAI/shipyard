...
async function handleApprovalState(
	...
	if (existingApproval && existingApproval.ownerId !== userId) {
		platform.warn(
			'[handleApprovalState] Rejected: sender is not owner',
	...
	if (!existingApproval && message.ownerId !== userId) {
		platform.warn(
			'[handleApprovalState] Rejected: sender claims to be different owner',
	...
	platform.info(
		'[handleApprovalState] Approval state updated',
		{
			planId: message.planId,

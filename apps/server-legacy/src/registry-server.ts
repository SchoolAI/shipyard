...
async function tryRemoveStaleLock(
	...
	try {
		...
		logger.error(
			{ err: unlinkErr, stalePid, retryCount },
			"Failed to remove stale hub lock",
		);
		return false;
	}
}
...
function tryRecoverStaleLock(originalError: Error): boolean {
	...
	try {
		...
		if (isProcessAlive(pid)) {
			logger.error(
				{ holderPid: pid },
				"LevelDB locked by active process, cannot recover",
			);
			throw originalError;
		}
...
function initPersistence(): void {
	...
	try {
		...
		if (!(err instanceof Error)) {
			logger.error(
				{ err },
				"Failed to initialize LevelDB persistence with unknown error",
			);
...
function shouldRejectForEpoch(doc: Y.Doc, planId: string): boolean {
	...
	if (planId === "plan-index") {
		...
		if (!metadata) {
			logger.warn(
				{ planId },
				"Plan-index metadata missing - rejecting for security",
			);
			return true;
		}
		...
		if (!isEpochValid(planEpoch, minimumEpoch)) {
			logger.warn(
				{ planId, planEpoch, minimumEpoch },
				"Plan-index epoch below minimum",
			);
			return true;
		}
		return false;
		...
		if (docName === "plan-index") {
			...
			if (!metadata) {
				...
				logger.info(
					...
					"Initialized plan-index metadata",
				);
			}
		}
...
function handleWebSocketConnection(
	...
	logger.info(
		{ planId, clientEpoch },
		"WebSocket client connected to registry",
	);
	...
	(async () => {
		try {
			...
			if (pendingMessages.length > 0) {
				logger.debug(
					{ planId, count: pendingMessages.length },
...
async function handleGetPRDiff(req: Request, res: Response): Promise<void> {
	...
	try {
		...
		logger.debug(
			{ planId, prNumber: prNum, repo: metadata.repo },
			"Served PR diff",
		);
	} catch (error) {
...
async function handleGetPRFiles(req: Request, res: Response): Promise<void> {
	...
	try {
		...
		logger.debug(
			{ planId, prNumber: prNum, fileCount: fileList.length },
			"Served PR files",
		);
	} catch (error) {
...
async function handleGetTranscript(req: Request, res: Response): Promise<void> {
	...
	try {
		...
		if (metadata.origin.platform !== "claude-code") {
			res
				.status(400)
				.json({ error: "Transcript only available for Claude Code plans" });
			return;
		}
		...
		logger.debug(
			{ planId, transcriptPath, size: content.length },
			"Served transcript for handoff",
		);
	} catch (error) {
...
function createPlanStore(): PlanStore {
	return {
		...
		deleteSubscription: (planId, clientId) =>
			deleteSubscription(planId, clientId),
	...
	app.get(
		...
		async (req: Request, res: Response): Promise<void> => {
			...
			const ARTIFACTS_DIR = join(
				registryConfig.SHIPYARD_STATE_DIR,
				"artifacts",
			);

...
> {
	...
	if (!metadata) {
		return {
			success: false,
			response: errorResponse(`Task "${taskId}" not found.`),
		};
	}
...
- diff: Code changes, git diffs`,
		inputSchema: {
			...
			properties: {
				...
				filename: {
					type: "string",
					description:
						"Filename with extension (e.g., screenshot.png, demo.mp4)",
				},
				source: {
					type: "string",
				...
				description: {
					type: "string",
					description:
						"Human-readable description of what this artifact proves",
				},
				deliverableId: {
					type: "string",
					description:
	...
	handler: async (args: unknown) => {
		...
		const validation = await validateSessionAndGetMetadata(
			taskId,
			sessionToken,
		);
...
async function processArtifactUpload(params: {
	...
	try {
		...
		logger.info(
			{ taskId, artifactId: artifact.id, url: artifactUrl },
			"Artifact added",
		);
...
function handleDeliverableLinking(
	...
	const linked = linkArtifactToDeliverable(
		doc,
		deliverableId,
		artifactId,
		actorName,
	...
	logger.info(
		{ taskId, artifactId, deliverableId },
	...
	if (!transitionResult.success) {
		logger.warn(
			{ taskId, error: transitionResult.error },
			"Failed to auto-progress status",
		);
		return false;
	}

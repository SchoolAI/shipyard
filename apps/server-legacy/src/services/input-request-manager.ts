...
export class InputRequestManager {
	...
	async createRequest(
		...
		ydoc.transact(() => {
			const requestsArray = ydoc.getArray<InputRequest>(
	...
	async waitForResponse(
		...
		return new Promise((resolve) => {
			...
			const handleAnsweredStatus = (request: AnyInputRequest) => {
				logger.info(
					{ requestId, answeredBy: request.answeredBy },
				...
				const responseValue =
					request.type === "multi" ? request.responses : request.response;
			...
			const handleTimeout = () => {
				...
				logger.warn(
					{ requestId, timeout: effectiveTimeout },
		...
		ydoc.transact(() => {
			...
			if (index !== -1) {
				...
				if (request.status === "pending") {
					...
					logger.debug(
						{ requestId, status: request.status },
						"Request not pending, cannot cancel",
					);
				}
			} else {
	...
	getRequest(ydoc: Y.Doc, requestId: string): InputRequest | undefined {
		...
		if (request) {
			...
			logger.warn(
				{ requestId, error: parseResult.error },
				"Invalid request data in Y.Doc",
			);
		}
		...
		ydoc.transact(() => {
			...
			for (let i = requests.length - 1; i >= 0; i--) {
				...
				if (
					...
					(request.answeredAt ?? request.createdAt) < cutoff
				) {

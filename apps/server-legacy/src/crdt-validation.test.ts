...
describe("CRDT Validation", () => {
	...
	describe("validateYDoc", () => {
		...
		it("should detect invalid metadata status", () => {
			...
			const metadataResult = report.results.find(
				(r) => r.key === YDOC_KEYS.METADATA,
			);
		...
		it("should detect invalid artifact in array", () => {
			...
			expect(report.isCorrupted).toBe(true);
			const artifactsResult = report.results.find(
				(r) => r.key === YDOC_KEYS.ARTIFACTS,
			);
		...
		it("should detect invalid linked PR", () => {
			...
			expect(report.isCorrupted).toBe(true);
			const prsResult = report.results.find(
				(r) => r.key === YDOC_KEYS.LINKED_PRS,
			);
		...
		it("should detect invalid deliverable", () => {
			...
			expect(report.isCorrupted).toBe(true);
			const delResult = report.results.find(
				(r) => r.key === YDOC_KEYS.DELIVERABLES,
			);
		...
		it("should detect invalid input request", () => {
			...
			expect(report.isCorrupted).toBe(true);
			const requestsResult = report.results.find(
				(r) => r.key === YDOC_KEYS.INPUT_REQUESTS,
			);

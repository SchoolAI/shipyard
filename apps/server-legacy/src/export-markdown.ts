 ...
 */
function extractThreadTextFromFragment(
 ...
 */
function extractThreadIdFromAttrs(
...
function formatThread(
	...
	thread.comments.forEach((comment, idx) => {
		...
		const authorName = resolveUser
			? resolveUser(comment.userId)
 ...
 */
function groupInputRequestEvents(
	events: PlanEvent[],
): Map<string, InputRequestGroup> {
 ...
 */
function formatInputRequestGroup(
	requestId: string,
	group: InputRequestGroup,
	num: number,
): string {
...
function formatAgentActivities(events: PlanEvent[]): string {
	const agentActivities = events.filter(
		(e): e is PlanEvent & { type: "agent_activity" } =>

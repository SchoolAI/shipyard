 ...
 */
export const DEFAULT_TIER_THRESHOLDS: Record<FanInResult['tier'], CoverageThresholds> = {
...
export function generateCoverageThresholds(
  srcDir: string,
  tierThresholds: Record<FanInResult['tier'], CoverageThresholds> = DEFAULT_TIER_THRESHOLDS,
  enabled = true
...
const DEFAULT_EXCLUDE = [/\.test\.ts$/, /\.spec\.ts$/, /\.d\.ts$/, /__tests__/, /node_modules/];

/** Virtual fan-in boost for public API files */
const PUBLIC_API_BOOST = 10;

function getAllTsFiles(dir: string, exclude: RegExp[], files: string[] = []): string[] {
...
function resolveImport(
  ...
  for (const [alias, replacement] of Object.entries(aliases)) {
    ...
    if (importPath.startsWith(aliasPrefix)) {
      const resolved = importPath.replace(aliasPrefix, replacement.replace('*', ''));
...
if (process.argv[1]?.endsWith('analyze-fan-in.ts')) {
  ...
  const sorted = [...analysis.results.values()].sort((a, b) => b.fanIn - a.fanIn);
  ...
  console.log('   Run tests to see coverage (see tests/integration-coverage.test.ts)\n');
  ...
  console.log(`ℹ️  ${sorted.length - sharedInfra.length} files use global defaults`);

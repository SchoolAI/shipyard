...
export type P2PMessageTypeValue =
	(typeof P2PMessageType)[keyof typeof P2PMessageType];
...
export type ConversationExportStartMeta = z.infer<
	typeof ConversationExportStartMetaSchema
>;
...
export function isConversationExportStart(data: Uint8Array): boolean {
	return (
		data.length > 0 && data[0] === P2PMessageType.CONVERSATION_EXPORT_START
	);
}
 ...
 */
export function encodeExportStartMessage(
 ...
 */
export function decodeExportStartMessage(
	data: Uint8Array,
): ConversationExportStartMeta {
	if (
		data.length === 0 ||
		data[0] !== P2PMessageType.CONVERSATION_EXPORT_START
	) {
...
export function encodeChunkMessage(chunk: ChunkMessage): Uint8Array {
	...
	const result = new Uint8Array(
		1 + 4 + exportIdBytes.length + 4 + chunk.data.length,
	);
	let offset = 0;
...
export function decodeChunkMessage(data: Uint8Array): ChunkMessage {
	...
	const exportId = textDecoder.decode(
		data.slice(offset, offset + exportIdLength),
	);
 ...
 * @throws {Error} If the message is malformed or validation fails
 */
export function decodeExportEndMessage(
	data: Uint8Array,
...
export function isP2PAgentLaunchMessage(data: Uint8Array): boolean {
	...
	const type = data[0];
	return (
		type === P2PMessageType.AGENT_LAUNCH_REQUEST ||
		type === P2PMessageType.AGENT_LAUNCH_RESPONSE
	);
}
 ...
 * Format: [type byte (1)] [JSON payload]
 */
export function encodeAgentLaunchRequest(
 ...
 * Format: [type byte (1)] [JSON payload]
 */
export function encodeAgentLaunchResponse(
	response: AgentLaunchResponse,
): Uint8Array {
 ...
 * @throws {Error} If the message is malformed or validation fails
 */
export function decodeAgentLaunchResponse(
	data: Uint8Array,
): AgentLaunchResponse {
 ...
 */
export function decodeP2PAgentMessage(
	data: Uint8Array,
	...
	switch (type) {
		case P2PMessageType.AGENT_LAUNCH_REQUEST:
			return {
				type: "agent_launch_request",
				payload: decodeAgentLaunchRequest(data),
			};
		case P2PMessageType.AGENT_LAUNCH_RESPONSE:
			return {
				type: "agent_launch_response",
				payload: decodeAgentLaunchResponse(data),
			};
		default:

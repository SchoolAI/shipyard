...
describe("PR linking helpers", () => {
	...
	describe("updateLinkedPRStatus", () => {
		...
		it("preserves other PR fields when updating status", () => {
			linkPR(
				ydoc,
				createPR(42, { title: "My PR", branch: "main", status: "draft" }),
			);
		...
		it("handles all valid status values", () => {
			const statuses: LinkedPR["status"][] = [
				"draft",
				"open",
				"merged",
				"closed",
...
describe("Deliverables helpers", () => {
	...
	describe("linkArtifactToDeliverable", () => {
		it("returns false when deliverable does not exist", () => {
			expect(linkArtifactToDeliverable(ydoc, "nonexistent", "art-1")).toBe(
				false,
			);
		});
...
describe("Plan metadata helpers", () => {
	...
	describe("initPlanMetadata", () => {
		...
		it("sets origin metadata when provided", () => {
			...
			const map = ydoc.getMap("metadata");
			const origin = map.get("origin") as {
				platform: string;
				sessionId: string;
			};
...
describe("Change Snapshot helpers", () => {
	...
	it("markMachineDisconnected preserves other snapshot fields", () => {
		const snapshot = createSnapshot("machine-1", {
			branch: "feature/test",
			files: [
				{ path: "file.ts", status: "modified", patch: "", staged: false },
			],
			totalAdditions: 10,
			totalDeletions: 5,
		});
...
describe("Thread helpers", () => {
	...
	describe("getThread", () => {
		...
		it("filters out invalid threads when searching", () => {
			...
			const validThread = {
				id: "valid-thread",
				comments: [
					{ id: "c1", userId: "user1", body: "Valid", createdAt: Date.now() },
				],
			};
	...
	describe("parseThreadId", () => {
		...
		it("extracts ID from wrapped format [thread:id]", () => {
			...
			expect(
				parseThreadId("[thread:0e63dc87-28ab-4587-8c6b-029216f33ced]"),
			).toBe("0e63dc87-28ab-4587-8c6b-029216f33ced");
		});
	...
	describe("parseCommentId", () => {
		...
		it("handles UUID-style IDs", () => {
			...
			expect(parseCommentId(`pr:${uuid}`)).toEqual({ type: "pr", id: uuid });
			expect(parseCommentId(`[local:${uuid}]`)).toEqual({
				type: "local",
				id: uuid,
			});
		});
		...
		it("handles edge cases", () => {
			...
			/** Partial wrapper */
			expect(parseCommentId("[abc123]")).toEqual({
				type: "unknown",
				id: "[abc123]",
			});

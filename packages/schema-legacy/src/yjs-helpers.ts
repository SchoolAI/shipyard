 ...
 */
export function getPlanMetadataWithValidation(
	ydoc: Y.Doc,
 ...
 */
function applyDraftTransition(
	map: Y.Map<unknown>,
	_transition: TransitionToDraft,
): void {
 ...
 */
function applyInProgressTransition(
	map: Y.Map<unknown>,
	transition: TransitionToInProgress,
): void {
	map.set("reviewedAt", transition.reviewedAt);
	map.set("reviewedBy", transition.reviewedBy);
	if (transition.reviewComment !== undefined) {
...
function applyCompletedTransition(
	map: Y.Map<unknown>,
	transition: TransitionToCompleted,
): void {
 ...
 */
function applyStatusTransitionFields(
	map: Y.Map<unknown>,
 ...
 */
export function resetPlanToDraft(
	ydoc: Y.Doc,
	actor?: string,
): TransitionResult {
...
export function initPlanIndexMetadata(
	ydoc: Y.Doc,
	init: InitPlanIndexMetadataParams = {},
): void {
	const map = ydoc.getMap(YDOC_KEYS.METADATA);
	const now = Date.now();
	...
	ydoc.transact(
		() => {
			...
			const index = existing.findIndex(
				(p) => p.prNumber === validated.prNumber,
			);
...
export function getPRReviewCommentsForPR(
	ydoc: Y.Doc,
	prNumber: number,
): PRReviewComment[] {
...
export function addPRReviewComment(
	...
	ydoc.transact(
		() => {
			const array = ydoc.getArray<PRReviewComment>(
...
export function resolvePRReviewComment(
	ydoc: Y.Doc,
	commentId: string,
	resolved: boolean,
): boolean {
 ...
 */
export function getLocalDiffCommentsForFile(
	ydoc: Y.Doc,
	path: string,
): LocalDiffComment[] {
 ...
 */
export function addLocalDiffComment(
	ydoc: Y.Doc,
 ...
 */
export function removeLocalDiffComment(
	ydoc: Y.Doc,
	commentId: string,
): boolean {
	const array = ydoc.getArray<LocalDiffComment>(YDOC_KEYS.LOCAL_DIFF_COMMENTS);
 ...
 */
export function getPRReviewCommentById(
	ydoc: Y.Doc,
	commentId: string,
): PRReviewComment | null {
 ...
 * @returns The comment if found, null otherwise.
 */
export function getLocalDiffCommentById(
	ydoc: Y.Doc,
	commentId: string,
): LocalDiffComment | null {
...
function extractViewedByFromCrdt(
	existingViewedBy: unknown,
...
export function markPlanAsViewed(ydoc: Y.Doc, username: string): void {
	...
	ydoc.transact(() => {
		...
		const viewedBy: Record<string, number> =
			extractViewedByFromCrdt(existingViewedBy);
 ...
 */
export function getAllTagsFromIndex(
	indexEntries: Array<{ tags?: string[] }>,
): string[] {
...
export function answerInputRequest(
	...
	const requestsArray = ydoc.getArray<AnyInputRequest>(
...
export function atomicRegenerateTokenIfOwner(
	...
	let result: AtomicRegenerateTokenResult = {
		success: false,
	...
	ydoc.transact(
		() => {
			...
			const currentOwner =
				typeof rawOwnerId === "string" ? rawOwnerId : undefined;

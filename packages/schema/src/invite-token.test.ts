...
describe("parseInviteFromUrl", () => {
	...
	it("should handle invite param with colons in token value", () => {
		const url =
			"https://example.com/task/abc123?invite=token123:secret:with:colons";
		const result = parseInviteFromUrl(url);
...
describe("buildInviteUrl", () => {
	it("should build correct invite URL", () => {
		...
		expect(url).toBe(
			"https://example.com/task/plan123?invite=token456%3Asecret789",
		);
	});
	...
	it("should handle baseUrl with subdirectory path", () => {
		...
		expect(url).toBe(
			"https://example.com/shipyard/task/plan123?invite=token456%3Asecret789",
		);
	});

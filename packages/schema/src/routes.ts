...
export const ROUTES = {
	...
	PLAN_HAS_CONNECTIONS: (planId: string) =>
		`/api/plan/${planId}/has-connections`,
	...
	PLAN_PR_DIFF: (planId: string, prNumber: number) =>
		`/api/plans/${planId}/pr-diff/${prNumber}`,
	PLAN_PR_FILES: (planId: string, prNumber: number) =>
		`/api/plans/${planId}/pr-files/${prNumber}`,
	...
	HOOK_SESSION_TOKEN: (planId: string) =>
		`/api/hook/plan/${planId}/session-token`,

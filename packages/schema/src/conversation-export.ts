...
export const A2AMessageSchema = z
	...
	.refine(
		...
		{
			message:
				"Invalid parts array - each part must have valid type and required fields",
			path: ["parts"],
		},
	)
	.transform((val) => {
...
export type ConversationExportMeta = z.infer<
	typeof ConversationExportMetaSchema
>;
...
type ClaudeCodeToolResultBlock = z.infer<
	typeof ClaudeCodeToolResultBlockSchema
>;
		...
		switch (val.type) {
			...
			case "tool_result":
				return {
					...
					is_error:
						typeof record.is_error === "boolean" ? record.is_error : undefined,
				};
			default: {
 ...
 */
export function parseClaudeCodeTranscriptString(
	content: string,
...
function isToolDataPart(part: A2ADataPart): boolean {
	...
	return Boolean(
		data &&
			typeof data === "object" &&
			("toolUse" in data || "toolResult" in data),
	);
}
...
function convertA2APartToContentBlock(part: A2APart): ClaudeCodeContentBlock[] {
	switch (part.type) {
		...
		default: {
			...
			throw new Error(
				`Unhandled part type: ${JSON.stringify(_exhaustiveCheck)}`,
			);
		}
	}
}
 ...
 */
export function a2aToClaudeCode(
	messages: A2AMessage[],
	sessionId?: string,
): ClaudeCodeMessage[] {
	...
	return messages.map((msg) => {
		const claudeMsg = convertA2AToClaudeCodeMessage(
			msg,
			resolvedSessionId,
			parentUuid,
		);

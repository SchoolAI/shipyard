...
describe("Export Start Message", () => {
	...
	it("throws on decode of wrong type", () => {
		...
		expect(() => decodeExportStartMessage(wrongType)).toThrow(
			"wrong type byte",
		);
	});

	it("throws on decode of empty message", () => {
		expect(() => decodeExportStartMessage(new Uint8Array([]))).toThrow(
			"wrong type byte",
		);
	});
...
describe("decodeP2PMessage", () => {
	...
	it("throws on empty message", () => {
		expect(() => decodeP2PMessage(new Uint8Array([]))).toThrow(
			"Cannot decode empty message",
		);
	});
});
...
describe("assertNeverP2PMessage", () => {
	it("throws with descriptive error", () => {
		...
		expect(() => assertNeverP2PMessage(fakeNever)).toThrow(
			"Unhandled P2P message type",
		);
	});
});

...
/**
 * Loro frontier operation ID shape.
 * Used for time travel - stores the document state at a specific point.
 * Matches the Frontiers type from @loro-extended/change: { peer: PeerID; counter: number }[]
 */
const FrontierOpIdShape = Shape.plain.struct({
  peer: Shape.plain.string(),
  counter: Shape.plain.number(),
});
...
export const TaskEventShape = Shape.plain.discriminatedUnion('type', {
  ...
  approved: Shape.plain.struct({
    ...
    /** Loro frontier at the time of approval - enables time travel to view content at this point */
    frontier: Shape.plain.array(FrontierOpIdShape).nullable(),
  }),
  changes_requested: Shape.plain.struct({
    type: Shape.plain.string('changes_requested'),
    ...EventBaseFields,
    message: Shape.plain.string().nullable(),
    /** Loro frontier at the time of request - enables time travel to view content at this point */
    frontier: Shape.plain.array(FrontierOpIdShape).nullable(),
  }),
...
export const TaskDocumentSchema = Shape.doc({
  meta: Shape.struct({
    ...
    /**
     * Access control fields for shared tasks.
     * When approvalRequired is true, users must be in approvedUsers to view content.
     * Owner is always approved implicitly.
     */
    approvalRequired: Shape.plain.boolean(),
...
type TaskDeliverableInternal = Infer<typeof TaskDocumentSchema.shapes.deliverables>;
type TaskLinkedPRInternal = Infer<typeof TaskDocumentSchema.shapes.linkedPRs>;
type TaskInputRequestInternal = Infer<typeof TaskDocumentSchema.shapes.inputRequests>;

export type TaskEventItem = Infer<typeof TaskEventShape>;
type ChangeSnapshotInternal = Infer<typeof TaskDocumentSchema.shapes.changeSnapshots>;
...
/**
 * Loro frontier operation ID - identifies a point in document history.
 * Used for time travel to view historical content at review points.
 */
export type FrontierOpId = Infer<typeof FrontierOpIdShape>;

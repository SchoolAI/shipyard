...
import { loro, type TypedDoc } from '@loro-extended/change';
import type { EventId, TaskId } from '../ids.js';
import { generateEventId } from '../ids.js';
import type { FrontierOpId, RoomShape, TaskDocumentShape, TaskEventItem } from '../shapes.js';
 ...
 */
type EventDataFor<T extends EventType> = Omit<Extract<TaskEventItem, { type: T }>, EventAutoFields>;
 ...
 */
type RequiresData<T extends EventType> = keyof EventDataFor<T> extends never ? false : true;
...
export class TaskDocument {
  ...
  constructor(taskDoc: TypedDoc<TaskDocumentShape>, roomDoc: TypedDoc<RoomShape>, taskId: TaskId) {
  ...
  syncPendingRequestsToRoom(): void {
    ...
    const hasPending = requests.some((req: { status: string }) => req.status === 'pending');
  ...
  logEvent<T extends EventType>(
    ...
    /** Capture Loro frontier for approved/changes_requested events to enable time travel */
    let frontier: FrontierOpId[] | null = null;
    if (type === 'approved' || type === 'changes_requested') {
      const rawFrontiers = loro(this.#taskDoc).doc.frontiers();
      frontier = rawFrontiers.map((f) => ({
        peer: String(f.peer),
        counter: f.counter,
      }));
    }
